...
class API_Testing(APITestCase):
    ...
    def test_put_invalid_address(self):    
        ...
        self.assertIn('city field cannot be less than three',response.json()['non_field_errors'][0])
        self.assertIn('country field cannot be less than three',response.json()['non_field_errors'][0])
        self.assertIn('house_num field cannot be less than zero',response.json()['non_field_errors'][0])
        self.assertIn('street field there must be an inscription street',response.json()['non_field_errors'][0])
        self.assertIn('zip_code field cannot be five digits',response.json()['non_field_errors'][0])
